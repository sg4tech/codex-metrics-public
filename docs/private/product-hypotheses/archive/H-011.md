...
**2026-04-18 — Closed: rejected as a user-facing concern.** Founder confirmed that `start-task` / `finish-task` / the active-task concept are not user-facing product features and never will be. They exist only as agent-internal workflow gates inside this repo (self-dogfood). No future iteration should treat manual-task enforcement as a product problem. Any residual value of the enforcement spec belongs to the agent-workflow layer, not to the product. See `docs/private/product-strategy.md` amendment 2026-04-18 (final lock).
